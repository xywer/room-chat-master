...
app.get('/registrePersonaInformacion', function (req, res, next) {
    ...
    if (!post.id) {//crear nuevo
        var table_save = "persona";
        var data_save = {nombres: post.nombres, apellidos: post.apellidos, persona_genero_id: post.genero_id};
        var query_save_persona = connection.query('INSERT INTO ' + table_save + ' SET ?', data_save, function (err, result) {
            var table_save = "persona_informacion";
            var persona_id = result.insertId;
            console.log(result, err);
            var data_save = {compania_taxis: post.compania_taxis, ciudad: post.ciudad, persona_id: persona_id};
            var query_save_persona_informacion = connection.query('INSERT INTO ' + table_save + ' SET ?', data_save, function (err, result2) {
                var children_id = result2.insertId;
                var data_result = {id: children_id, nombres: post.nombres, apellidos: post.apellidos, persona_genero_id: post.persona_genero_id, compania_taxis: post.compania_taxis, ciudad: post.ciudad, persona_id: persona_id};
                res.json({success: true, data: data_result, msj: 'Se registro correctamente!!'});
            });
        });

    } else {
        var queryString = 'UPDATE  persona SET nombres="' + post.nombres + '",' + 'apellidos="' + post.apellidos + '",' + 'persona_genero_id="' + post.persona_genero_id + '" WHERE persona.id=' + post.id;
        connection.query(queryString, function (err, result) {

        });
        var queryString = 'UPDATE  persona_informacion SET compania_taxis="' + post.compania_taxis + '",' + 'ciudad="' + post.ciudad + '" WHERE id.id=' + post.persona_id;
        connection.query(queryString, function (err, result) {
...
app.get('/getPersonaInformacion', function (req, res, next) {
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var query_string = "SELECT t.id id,t.persona_id, pg.valor genero,p.nombres,p.apellidos ,t.compania_taxis,t.ciudad FROM persona_informacion  t ";
